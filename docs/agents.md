...
`runner` is `"pi"` or `"claude"` and defaults to `agents.runner` (`"pi"`). Model resolution for a Pi child is: explicit `model` > `tier` > (when `agents.inheritParentModel` is on, the default) the parent session's active model > `agents.model`. So by default a Pi child that names no `model`/`tier` runs on the same model as the session that spawned it, and because each level reads its own live session model this inheritance propagates recursively to nested agents; set `agents.inheritParentModel: false` to fall back to the static `agents.model` instead. Claude children use `agents.claude.model` or Claude Code's own runtime default and never inherit a Pi model string. Their tool allowlist defaults to `agents.defaultTools`. Reasoning effort defaults to the inherited parent thinking level (or `agents.thinking`, `medium`, when inheritance is off or unavailable) unless the call sets `thinking`; Pi clamps it to model support, while Claude forwards it through `--effort` (`off`/`minimal` map to `low`).
...
`rlm.query()` is `agents.run({ runner: "pi", recursive: true })` with Fabric enabled in the child. Claude runners are intentionally rejected for recursive Fabric. Recursion is rejected at `agents.maxDepth`. Approval of the initial recursive call delegates only the `agent` risk capability to recursive children; network, execution, and write approvals are not inherited. Each Fabric process enforces its own configured concurrency and timeout limits. When `agents.budgetUsd` is set, a shared append-only cost ledger bounds total spend across the whole recursion tree: every node records the cost of the children it spawns into one ledger file inherited via environment, and each node rejects a new child when the accumulated spend reaches the budget. Accounting is reserve-then-settle: at spawn time a node appends a `reservation` entry (an estimate from `agents.reserveEstimateUsd`) and re-reads the ledger before committing, then on completion appends a `settlement` entry that supersedes the reservation with the actual cost. Because the pre-spawn check counts settled cost plus every outstanding reservation, concurrent children can no longer all pass before any cost lands. Reservations bound the concurrency race but not the estimation error, so a tree can still overshoot by roughly the reserve estimate; the race-free ceiling remains `agents.maxPerExecution`. Legacy ledgers written before reservations existed (entries with no `kind` field) are read as settled costs, so the format stays backward compatible. `agents.budgetEnforcement` selects how the ceiling is enforced: `soft` (default) only refuses new spawns; `hard` additionally stops the children already running through the normal stop path once settled + reserved cost reaches the budget. The result and live status of every recursive child carry a `budget` summary (`limit`, `spent`, `remaining`, `tokens`). Fabric also keeps the latest bounded nested-agent status tree in memory, so completed recursive leaves remain visible in **Topology · Run** after the child process removes its temporary nested run directories. The snapshot is released when the parent run is cleaned up or the Fabric session shuts down.
