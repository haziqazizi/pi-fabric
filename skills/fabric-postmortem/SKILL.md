    ...
    try {
      return await agent<Diagnosis>(
        `You are analyzing why an agent run underperformed. Scope: ${π.scope}\n\n` +
          ...
          `\n\nIdentify recurring FAILURE PATTERNS: behaviors that wasted work or produced wrong results (e.g. brute-force loops that timed out, unverified sub-results built upon, re-reading the same files, claims without tool evidence, missing error handling for a specific tool). For each: the pattern, verbatim evidence, and ONE actionable steering tip a future prompt could carry. Tips must be strategy-level and reusable — never task-specific facts, file names as answers, or anything secret.`,
        {
